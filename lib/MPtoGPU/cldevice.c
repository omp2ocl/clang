...
// COPYLEFT
//   Copyleft (C) 2015--2017, UNICAMP & Samsumg R&D
...
// max work items in each dimmension for current device (_clid)
int _max_work_items[3] = {128, 1, 1};
...
void _cldevice_details(cl_device_id id, cl_device_info param_name,
...
void _cldevice_init(int rtlmode) {
  ...
  if (_device == NULL) {
    ...
    _status = clGetPlatformInfo(_platform, CL_PLATFORM_NAME,
    ...
    _status =
        clGetPlatformInfo(_platform, CL_PLATFORM_EXTENSIONS,
    ...
    if (extStringStart != 0) {
      ...
      if (_verbose)
        printf("<rtl> This platform does not support cl_khr_spir extension.\n");
      _spir_support = 0;
    }

    // Fetch the device list for this platform
    _status =
        clGetDeviceIDs(_platform, CL_DEVICE_TYPE_ALL, 0, NULL, &_ndevices);
    ...
    _cmd_queue =
        (cl_command_queue *)calloc(_ndevices, sizeof(cl_command_queue));
    _gpu_present = 0;
    ...
    if (_status == CL_SUCCESS) {
      ...
      _status =
          clGetDeviceIDs(_platform, CL_DEVICE_TYPE_CPU, 1, &_device[idx], NULL);
      if (_status != CL_SUCCESS) {
    ...
    if (_ndevices > idx) {
      ...
      if (_status == CL_SUCCESS) {
        ...
        _status = clGetDeviceIDs(_platform, CL_DEVICE_TYPE_GPU, ndev,
                                 &_device[idx], NULL);
        if (_status != CL_SUCCESS) {
    ...
    if (_ndevices > idx) {
      ...
      _status =
          clGetDeviceIDs(_platform, CL_DEVICE_TYPE_ACCELERATOR, 0, NULL, &ndev);
      if (_status == CL_SUCCESS) {
        if (_verbose) {
        ...
        _status = clGetDeviceIDs(_platform, CL_DEVICE_TYPE_ACCELERATOR, ndev,
                                 &_device[idx], NULL);
        if (_status != CL_SUCCESS) {
          fprintf(stderr,
    ...
    if (_ndevices > idx) {
      ...
      _status =
          clGetDeviceIDs(_platform, CL_DEVICE_TYPE_DEFAULT, 0, NULL, &ndev);
      if (_status == CL_SUCCESS) {
        if (_verbose) {
        ...
        _status = clGetDeviceIDs(_platform, CL_DEVICE_TYPE_DEFAULT, ndev,
                                 &_device[idx], NULL);
        if (_status != CL_SUCCESS) {
    ...
    for (i = 0; i < _ndevices; ++i) {
      ...
      if (_verbose && _device[i] != NULL) {
        ...
        _cldevice_details(_device[i], CL_DEVICE_VENDOR_ID,
                          "CL_DEVICE_VENDOR_ID");
        _cldevice_details(_device[i], CL_DEVICE_MAX_MEM_ALLOC_SIZE,
                          "CL_DEVICE_MAX_MEM_ALLOC_SIZE");
        _cldevice_details(_device[i], CL_DEVICE_GLOBAL_MEM_CACHELINE_SIZE,
                          "CL_DEVICE_GLOBAL_MEM_CACHELINE_SIZE");
        _cldevice_details(_device[i], CL_DEVICE_GLOBAL_MEM_SIZE,
                          "CL_DEVICE_GLOBAL_MEM_SIZE");
        _cldevice_details(_device[i], CL_DEVICE_MAX_COMPUTE_UNITS,
                          "CL_DEVICE_MAX_COMPUTE_UNITS");
        _cldevice_details(_device[i], CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS,
                          "CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS");
        _cldevice_details(_device[i], CL_DEVICE_MAX_WORK_ITEM_SIZES,
                          "CL_DEVICE_MAX_WORK_ITEM_SIZES");
        _cldevice_details(_device[i], CL_DEVICE_MAX_WORK_GROUP_SIZE,
                          "CL_DEVICE_MAX_WORK_GROUP_SIZE");
        _cldevice_details(_device[i], CL_DEVICE_PREFERRED_VECTOR_WIDTH_FLOAT,
      ...
      if (_device[i] != NULL) {
        ...
        _context[i] =
            clCreateContext(NULL, 1, &_device[i], NULL, NULL, &_status);
        if (_status != CL_SUCCESS) {
          fprintf(stderr, "<rtl> Failed to create context for device %u.\n", i);
          _clErrorCode(_status);
        }

        // Create a command queue for each context to communicate with the
        // associated device
        _cmd_queue[i] =
            clCreateCommandQueue(_context[i], _device[i], properties, &_status);
        if (_status != CL_SUCCESS) {
          fprintf(stderr,
                  "<rtl> Failed to create commandQueue for device %u.\n", i);
          _clErrorCode(_status);
        }
      }
    }
...
///
cl_program _create_fromSource(cl_context context, cl_device_id device,
                              const char *fileName) {
  cl_int errNum;
  cl_program program;
  ...
  program =
      clCreateProgramWithSource(context, 1, (const char **)&buffer, NULL, NULL);
  if (program == NULL) {
...
///
cl_program _create_fromBinary(cl_context context, cl_device_id device,
                              const char *fileName) {
...
///
int _save_toBinary(cl_program program, cl_device_id device,
                   const char *fileName) {
...
///
cl_uint _get_num_devices() { return _ndevices; }
...
///
cl_uint _get_default_device() { return _clid; }
...
void _set_default_device(cl_uint id) {
  ...
  if ((id == 0) && (!_cpu_present)) {
    _clid = 1;
    fprintf(stderr,
            "<rtl> Warning: CPU is not set, run on device 1 instead.\n");
  }

  if (id == 1) {
    if (!_gpu_present && _cpu_present && _ndevices == 1) {
      _clid = 0;
      fprintf(stderr, "<rtl> Warning: Accelerator (GPU?) is not present, run "
                      "on CPU instead.\n");
    } else {
      _clid = id;
    }
    ...
    fprintf(stderr,
            "<rtl> Warning: Device id is invalid, run on device %u instead.\n",
            _clid);
  } else
    _clid = id;

  // Get the maximum number of work-items in each dimension for all devices
  status = clGetDeviceInfo(_device[_clid], CL_DEVICE_MAX_WORK_ITEM_SIZES, 0,
                           NULL, &param_size);
  if (status != CL_SUCCESS) {
    fprintf(
        stderr,
        "<rtl> Warning: Unable to obtain MAX_WORK_ITEM_SIZES for device %u.\n",
        _clid);
    return;
  }

  size_t *ret = (size_t *)alloca(sizeof(size_t) * param_size);
  status = clGetDeviceInfo(_device[_clid], CL_DEVICE_MAX_WORK_ITEM_SIZES,
                           param_size, ret, NULL);
  status = clGetDeviceInfo(_device[_clid], CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS,
                           sizeof(cl_uint), &maxWIDimensions, NULL);
  if (status != CL_SUCCESS) {
    fprintf(stderr,
            "<rtl> Warning: Unable to obtain MAX_WORK_ITEM_DIMENSIONS for "
            "device %u.\n",
            _clid);
    return;
  }
  ...
  if ((int)ret[i] > 2 * _work_group[j] * _work_group[j + 1] && _clid != 0) {
    _work_group[j] *= 2;
  }
  if ((int)ret[i] > 2 * _work_group[j] * _work_group[j + 1]) {
    if ((int)ret[i + 1] > 2 * _work_group[j + 1])
...
int _cl_create_write_only(uint64_t size) {
  _inc_curid();
  _locs[_curid] =
      clCreateBuffer(_context[_clid], CL_MEM_READ_WRITE, size, NULL, &_status);
  if (_status != CL_SUCCESS) {
    fprintf(stderr,
            "<rtl> Failed creating a %llu bytes write-only buffer on device.\n",
            size);
    _clErrorCode(_status);
    _curid--;
    return 0;
  ...
  if (_verbose)
    printf("<rtl> Creating a write-only buffer %d of size: %llu\n", _curid,
           size);
  return 1;
}
...
int _cl_offloading_write_only(uint64_t size, void *loc) {
  ...
  if (_status != CL_SUCCESS) {
    fprintf(stderr,
            "<rtl> Failed creating a %llu bytes write-only buffer %d.\n", size,
            _curid);
    _clErrorCode(_status);
    _curid--;
    return 0;
  ...
  if (_verbose)
    printf("<rtl> Creating a write-only buffer %d of %llu bytes\n", _curid,
           size);
  return 1;
}
...
int _cl_create_read_only(uint64_t size) {
  _inc_curid();
  _locs[_curid] =
      clCreateBuffer(_context[_clid], CL_MEM_READ_ONLY, size, NULL, &_status);
  if (_status != CL_SUCCESS) {
    fprintf(stderr,
            "<rtl> Failed creating a %llu read-only buffer on device.\n", size);
    _clErrorCode(_status);
    _curid--;
    return 0;
  }
  if (_verbose)
    printf("<rtl> Creating a read-only buffer %d of size: %llu\n", _curid,
           size);
  return 1;
}
...
int _cl_offloading_read_only(uint64_t size, void *loc) {
  ...
  _status =
      clEnqueueWriteBuffer(_cmd_queue[_clid], _locs[_curid], CL_TRUE, 0, size,
                           loc, 0, NULL, (_profile) ? &_global_event : NULL);
...
int _cl_create_read_write(uint64_t size) {
  ...
  if (_verbose)
    printf("<rtl> Creating a read-write buffer %d of size: %llu\n", _curid,
           size);
  return 1;
}
...
int _cl_read_buffer(uint64_t size, int id, void *loc) {
  ...
  _status =
      clEnqueueReadBuffer(_cmd_queue[_clid], _locs[id], CL_TRUE, 0, size, loc,
...
int _cl_write_buffer(uint64_t size, int id, void *loc) {
  ...
  _status =
      clEnqueueWriteBuffer(_cmd_queue[_clid], _locs[id], CL_TRUE, 0, size, loc,
                           0, NULL, (_profile) ? &_global_event : NULL);
...
///
/// Auxiliary Function.
/// Return true if program object was created before.
///
...
int _cl_create_program(char *str) {
  ...
  if (_does_file_exist(bc_file)) {
    ...
    _program[_kerid] =
        _create_fromBinary(_context[_clid], _device[_clid], bc_file);
    ...
    _program[_kerid] =
        _create_fromBinary(_context[_clid], _device[_clid], aocx_file);
  ...
  _program[_kerid] =
      _create_fromSource(_context[_clid], _device[_clid], cl_file);
  if (_program[_kerid] == NULL) {
...
int _cl_set_kernel_args(int nargs) {
  ...
  for (i = 0; i < nargs; i++) {
    ...
    if (_verbose)
      printf("<rtl> Pass buffer %d to kernel in pos %d\n", i, i);
  }
  return 1;
}
...
int _cl_set_kernel_arg(int pos, int index) {
  _status |=
      clSetKernelArg(_kernel[_kerid], pos, sizeof(cl_mem), &_locs[index]);
  if (_status != CL_SUCCESS) {
    fprintf(stderr, "<rtl> Error setting buffer %d to kernel in pos %d.\n",
            index, pos);
    _clErrorCode(_status);
...
///
int _cl_execute_kernel(uint64_t size1, uint64_t size2, uint64_t size3,
                       int dim) {
  ...
  global_size[0] = (size_t)ceil(((float)size1) / ((float)_work_group[idx])) *
                   _work_group[idx];
  global_size[1] =
      (size_t)ceil(((float)size2) / ((float)_work_group[idx + 1])) *
      _work_group[idx + 1];
  global_size[2] =
      (size_t)ceil(((float)size2) / ((float)_work_group[idx + 2])) *
      _work_group[idx + 2];
  ...
  _status = clEnqueueNDRangeKernel(_cmd_queue[_clid], _kernel[_kerid],
  ...
  if (_status == CL_SUCCESS) {
    ...
    if (_status == CL_INVALID_WORK_DIMENSION)
      fprintf(stderr, "<rtl> Error executing kernel. Number of dimmensions is "
                      "not a valid value.\n");
    else if (_status == CL_INVALID_GLOBAL_WORK_SIZE)
      fprintf(stderr, "<rtl> Error executing kernel. Global Work Size is NULL "
                      "or exceeded valid range.\n");
    else if (_status == CL_INVALID_WORK_GROUP_SIZE)
      fprintf(stderr, "<rtl> Error executing kernel. Local Work Size does not "
                      "match the Work Group size.\n");
    else if (_status == CL_INVALID_WORK_ITEM_SIZE)
      fprintf(stderr, "<rtl> Error executing kernel. The number of work-items "
                      "is greater than Max Work-items.\n");
    else
...
///
int _cl_execute_tiled_kernel(int wsize0, int wsize1, int wsize2, int block0,
                             int block1, int block2, int dim) {
  ...
  if (_status == CL_INVALID_WORK_DIMENSION)
    ...
    fprintf(stderr, "<rtl> Error executing kernel. Global Work Size is NULL or "
                    "exceeded valid range.\n");
  else if (_status == CL_INVALID_WORK_GROUP_SIZE)
    fprintf(stderr, "<rtl> Error executing kernel. Local Work Size does not "
                    "match the Work Group size.\n");
  else if (_status == CL_INVALID_WORK_ITEM_SIZE)
    fprintf(stderr, "<rtl> Error executing kernel. The number of work-items is "
...
void _cl_profile(const char *str, cl_event event) {
  ...
  _status = clGetEventProfilingInfo(event, CL_PROFILING_COMMAND_START,
  ...
  _status = clGetEventProfilingInfo(event, CL_PROFILING_COMMAND_END,
...
///
int _cl_get_threads_blocks(int *threads, int *blocks, int *sthreads,
                           int *sblocks, uint64_t size, int bytes) {
  int i, bytesthreads;
  size /= bytes;
  for (i = 0; i <= 10; i++) {
